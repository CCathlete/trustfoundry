"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.createRouter = void 0;
var express_1 = require("express");
var multer_1 = require("multer");
// Define the maximum number of files to accept in a single request.
var MAX_FILES_PER_REQUEST = 50;
// --- Multer Configuration ---
// Memory storage is used to get the file buffer for MinIO and validation
var storageConfig = multer_1.default.memoryStorage();
var upload = (0, multer_1.default)({
    storage: storageConfig,
    limits: {
        // 10 MB limit 
        fileSize: 10 * 1024 * 1024
    },
}).array('files', MAX_FILES_PER_REQUEST);
/**
 * Factory function to create and configure the application router.
 * Dependencies (IFileValidator, IStorageService) are injected here.
 * @param validator The service responsible for file validation.
 * @param storage The service responsible for file persistence.
 * @returns An Express Router instance.
 */
var createRouter = function (validator, storage) {
    var router = (0, express_1.Router)();
    /**
     * POST /upload
     * Multer middleware (`upload`) is used as a standard middleware.
     * If successful, it passes control to the async handler.
     * If it fails, it passes an error to the global error handler.
     */
    router.post('/upload', upload, function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var files, uploadPromises, fileNames, storageError_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    files = req.files;
                    if (!files || files.length === 0) {
                        return [2 /*return*/, res.status(400).json({ status: 'error', message: 'No files uploaded.' })];
                    }
                    // 1. Perform Batch Validation using injected IFileValidator
                    try {
                        validator.isOkToUpload(files);
                    }
                    catch (validationError) {
                        console.error('[Router] File Validation Error:', validationError);
                        return [2 /*return*/, res.status(400).json({
                                status: 'error',
                                message: validationError instanceof Error ? validationError.message : 'Files failed security validation.'
                            })];
                    }
                    console.log("[Router] Received ".concat(files.length, " files. Starting persistence..."));
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 3, , 4]);
                    uploadPromises = files.map(function (file) {
                        return storage.storeFile(file);
                    });
                    return [4 /*yield*/, Promise.all(uploadPromises)];
                case 2:
                    _a.sent();
                    console.log("[Router] Successfully persisted ".concat(files.length, " files to MinIO."));
                    fileNames = files.map(function (f) { return f.originalname; }).join(', ');
                    return [2 /*return*/, res.status(200).json({
                            status: 'success',
                            message: "Successfully processed and stored ".concat(files.length, " files: ").concat(fileNames)
                        })];
                case 3:
                    storageError_1 = _a.sent();
                    console.error('[Router] Storage Error:', storageError_1);
                    return [2 /*return*/, res.status(500).json({
                            status: 'error',
                            message: 'Failed to store files in the data lake.'
                        })];
                case 4: return [2 /*return*/];
            }
        });
    }); });
    /**
     * GET /status
     * Example of a future endpoint.
     */
    router.get('/status', function (_, res) {
        res.status(200).json({ status: 'ok', message: 'Data lake router is active.' });
    });
    return router;
};
exports.createRouter = createRouter;
